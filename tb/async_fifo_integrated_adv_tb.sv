...
module async_fifo_integrated_tb;
    ...
    task automatic drain_visible_read_data(input string check_name);
        begin
            while (empty !== 1'b1) begin
                fifo_read(got_data, ok);
                if (!ok) begin
                    $display("[ERROR] %s : residual drain failed at t=%0t", check_name, $time);
                    errors++;
                    break;
                end
            end
